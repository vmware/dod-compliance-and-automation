control 'WOAT-3X-000038' do
  title 'Workspace ONE Access must be configured with memory leak protection.'
  desc  "
    The Java Runtime environment can cause a memory leak or lock files under certain conditions. Without memory leak protection, Workspace ONE Access can continue to consume system resources which will lead to OutOfMemoryErrors when re-loading web applications.
  ...
  desc  'check', "
    ...
    # grep JreMemoryLeakPreventionListener /opt/vmware/horizon/workspace/conf/server.xml|sed 's/^ *//'
  ...
  tag gid: 'V-WOAT-3X-000038'
  tag rid: 'SV-WOAT-3X-000038'
  tag stig_id: 'WOAT-3X-000038'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

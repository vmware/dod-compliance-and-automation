control 'WOAD-3X-000010' do
  title 'The Workspace ONE Access vPostgres instance must be configured to enable a log statement of sufficient level.'
  ...
  desc  'check', "
    ...
    # /opt/vmware/vpostgres/9.6/bin/psql -U postgres -A -t -c \"SHOW log_statement\"
  ...
  desc 'fix', "
    ...
    # /opt/vmware/vpostgres/9.6/bin/psql -U postgres -c \"ALTER SYSTEM SET log_statement TO 'ddl';\"
  ...
  tag gid: 'V-WOAD-3X-000010'
  tag rid: 'SV-WOAD-3X-000010'
  tag stig_id: 'WOAD-3X-000010'
  tag cci: ['CCI-001464']
  tag nist: ['AU-14 (1)']
  ...
  if clustered
    describe command('/opt/vmware/vpostgres/9.6/bin/psql -U postgres -A -t -c "SHOW log_statement;"') do
      its('stdout.strip') { should cmp 'ddl' }
    end
  else

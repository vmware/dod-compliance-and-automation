control 'NALB-CO-000094' do
  title 'The VMware Avi Load Balancer Controller must obtain its public key certificates from an approved certificate authority.'
  desc  'For user certificates, each organization obtains certificates from an approved, shared service provider, as required by OMB policy. For federal agencies operating a legacy public key infrastructure cross-certified with the Federal Bridge Certification Authority at medium assurance or higher, this Certification Authority will suffice.'
  desc  'rationale', ''
  desc  'check', "
    Determine if the Avi Load Balancer obtains public key certificates from an appropriate certificate policy through an approved certificate authority.
  ...
  "
  desc 'fix', "
    To update the certificate used for the Avi Load Balancer Controller web interface do the following:

    If an approved certificate has not been imported, from the Avi Load Balancer Controller web interface go to Templates >> Security >> SSL/TLS Certificates.

    Click Create.

    Select either CSR to generate a certificate request to get signed or Import to import a new certificate and click Save.

    Note: To avoid any certificate trust issue, make sure the certificate chain is complete on the Avi Load Balancer Controller and on the client browser. Install the complete certificate chain (the root and the intermediate certificates) on the Avi Load Balancer Controller and on the client browser.

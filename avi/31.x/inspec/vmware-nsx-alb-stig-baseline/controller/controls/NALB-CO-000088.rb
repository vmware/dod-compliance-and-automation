control 'NALB-CO-000088' do
  title 'The VMware Avi Load Balancer Controller must disable basic authentication to the API.'
  desc  "
    API calls from a client to the Avi Load Balancer Controller must first be authenticated, either by HTTP session-based auth or HTTP basic auth. The use of HTTP basic authentication for API access is unrelated to the use of basic auth for clients accessing a virtual service in which the Service Engine is proxying the authentication.

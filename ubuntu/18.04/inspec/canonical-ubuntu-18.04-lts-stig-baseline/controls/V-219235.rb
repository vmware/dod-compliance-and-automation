control "V-219235" do
  title "The Ubuntu operating system must permit only authorized accounts to own the
    audit configuration files."
  desc "Without the capability to restrict which roles and individuals can select which
    events are audited, unauthorized personnel may be able to prevent the auditing of
    critical events. Misconfigured audits may degrade the system's performance by overwhelming
  ...
  tag "ia_controls": nil
  desc "check", "Verify that \"/etc/audit/audit.rules\", \"/etc/audit/rules.d/*\"
    and \"/etc/audit/auditd.conf\" files are owned by root account by using the following command:
    ...
    If \"/etc/audit/audit.rules\" or \"/etc/audit/rules.d/*\" or \"/etc/audit/auditd.conf\" file is owned by a user

control 'V-219310' do
  title "The Ubuntu operating system must immediately terminate all network connections
    associated with SSH traffic after a period of inactivity."
  ...
  desc 'check', "Verify that all network connections associated with SSH traffic
    ...
    Check that \"ClientAliveCountMax\" variable is set in \"/etc/ssh/sshd_config\"
    file by performing the following command:

    # sudo grep -i clientalivecountmax /etc/ssh/sshd_config

    ClientAliveCountMax 1

    If \"ClientAliveCountMax\" is not set, or not set to \"1\", or is
    commented out, this is a finding.
  "
  desc 'fix', "Configure the Ubuntu operating system to automatically terminate
    inactive SSH sessions after a period of inactivity.

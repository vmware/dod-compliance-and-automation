control 'V-219175' do
  ...
  tag "ia_controls": nil
  desc 'check', "Verify the Ubuntu operating system requires the change of at least 8
    characters when passwords are changed.

    Determine if the field \"difok\" is set in the \"/etc/security/pwquality.conf\" file
  ...
  desc 'fix', "Configure the Ubuntu operating system to require the change of at least 8
    characters when passwords are changed.

    Add or update the \"/etc/security/pwquality.conf\" file to include the \"difok=8\"
    parameter:

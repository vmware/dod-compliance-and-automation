control 'V-219344' do
  title "The Ubuntu operating system must be configured so that a file integrity
    tool verifies the correct operation of security functions every 30 days."
...
consoles, and/or hardware indications, such as lights.
  ...
  desc 'check', "Verify that Advanced Intrusion Detection Environment (AIDE)
    ...
    Note: A file integrity tool other than AIDE may be used, but the tool
    must be executed at least once per week.

    Check that AIDE is being executed every 30 days or less with the
    following command:
  ...
  "
  desc 'fix', "The cron file for AIDE is fairly complex as it creates the report.
    This file is installed with the aide-common package and the default can be
    restored by copying it from another location:

control 'V-219340' do
  ...
  desc 'check', "Verify an application firewall is configured to rate limit any
    ...
    Check that the Uncomplicated Firewall is configured to rate limit any connection to the
  ...
  "
  desc 'fix', "Configure the application firewall to protect against or limit the effects of
    Denial of Service (DoS) attacks by ensuring the Ubuntu operating system is implementing
    ...
    Or rate-limiting can be done on an interface. An example of adding a rate-limit on
    the eth0 interface:

control 'V-219307' do
  title "The Ubuntu operating system must implement DoD-approved
  ...
  tag "ia_controls": nil
  desc 'check', "Verify the SSH daemon is configured to only implement DoD-approved encryption.
    ...
    If no lines are returned, or the returned ciphers list contains any cipher not
    starting with \"aes\", this is a finding.
  "
  desc 'fix', "Configure the Ubuntu operating system to allow the SSH daemon to only
    implement DoD-approved encryption.

    Edit the SSH daemon configuration \"/etc/ssh/sshd_config\" and remove any ciphers not
    starting with \"aes\" and remove any ciphers ending with \"cbc\". If necessary, append the \"Ciphers\"
    line to the \"/etc/ssh/sshd_config\" document.

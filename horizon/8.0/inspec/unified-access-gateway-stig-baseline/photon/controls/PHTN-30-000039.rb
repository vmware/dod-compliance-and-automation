control 'PHTN-30-000039' do
  ...
  syslogtype = input('syslogtype', value: 'rsyslog')

  if syslogtype == 'rsyslog'
    describe file('/etc/rsyslog.conf') do
      its('content') { should match /^.*#{input('syslogServer')}.*$/ }
    end
  end

  if syslogtype == 'syslog-ng'
    describe command("syslog-ng --preprocess-into=/dev/stdout|&grep -i #{input('syslogServer')}") do
      its('stdout.strip') { should match /^.*#{input('syslogServer')}.*$/ }
    end
  end
end

control 'HZNV-8X-000006' do
  title 'The Horizon Connection Server user interface must display the Standard Mandatory DoD Notice and Consent Banner before granting access to the system.'
  desc  "
  ...
  desc  'check', "
    ...
    Scroll down to the \"Display a Pre-Login Message\" check box.

    If \"Display a Pre-Login Message\" is not checked, this is a finding.

    If the \"Display a Pre-Login Message\" field does not contain the Standard Mandatory DoD Notice and Consent Banner text, this is a finding.
  "
  desc 'fix', "
    Login to the Horizon Connection Server administrative interface as an administrator.
    ...
    Ensure the box next to \"Display a Pre-Login Message\" is checked.

    In the \"Display a Pre-Login Message\" field, supply the Standard Mandatory DoD Notice and Consent Banner text:
  ...
  tag gid: 'V-HZNV-8X-000006'
  tag rid: 'SV-HZNV-8X-000006'
  tag stig_id: 'HZNV-8X-000006'
  tag cci: ['CCI-000048']
  tag nist: ['AC-8 a']
  ...
  describe 'Checking if warning banner is configured' do
    subject { gensettings['display_pre_login_message'] }
    it { should cmp true }
  end

  if !gensettings['pre_login_message'].nil?
    describe 'Checking warning banner text' do
      subject { gensettings['pre_login_message'].gsub!(/\s/, '') }
      it { should cmp compareVal }
    end
  else
    describe 'Checking warning banner text' do
      subject { gensettings['pre_login_message'] }

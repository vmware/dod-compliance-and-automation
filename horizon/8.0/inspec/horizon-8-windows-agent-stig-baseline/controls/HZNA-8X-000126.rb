control 'HZNA-8X-000126' do
  ...
  desc  'check', "
    ...
    Double-click the \"CommandsToRunOnDisconnect\" setting.

    If \"CommandsToRunOnDisconnect\" is set to either \"Not Configured\" or \"Disabled\", this is not a finding.
  ...
  desc 'fix', "
    ...
    > Ensure \"CommandsToRunOnDisconnect\" is set to \"Disabled\".

    > Click \"OK\".

    Option 2:

    > Ensure \"CommandsToRunOnDisconnect\" is set to \"Enabled\".
  ...
  tag gid: 'V-HZNA-8X-000126'
  tag rid: 'SV-HZNA-8X-000126'
  tag stig_id: 'HZNA-8X-000126'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']

  horizonagenthelper.setconnection

  regexist = registry_key('HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\VMware, Inc.\\VMware VDM\\Agent\\Configuration\\CommandsToRunOnDisconnect')

  reg = json(command: '$path = "HKLM:\\SOFTWARE\\Policies\\VMware, Inc.\\VMware VDM\\Agent\\Configuration\\CommandsToRunOnDisconnect";
                          Get-Item -Path $path |
  ...
  describe.one do
    ...
    describe input('allowedDisconnectScripts') do

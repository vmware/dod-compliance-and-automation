control 'VRAA-8X-000007' do
  title 'VMware Aria Automation must perform log record management.'
  desc  "
  ...
  desc  'check', "
    ...
    # rotated hourly, or whenever a maximum size is exceeded. To ensure the max size triggers rotation
    # whenever needed, the services-logrotate timer triggers logrotate for this config multiple times
    ...
    /var/log/services-logs/*/untagged-apps/*.log /services-logs/*/untagged-apps/console-logs/*.log {
    }
    ...
    Then verify that each of them is covered by an extra configuration file located here:

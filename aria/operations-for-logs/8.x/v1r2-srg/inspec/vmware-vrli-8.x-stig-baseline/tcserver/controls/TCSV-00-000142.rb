control 'TCSV-00-000142' do
  title ' tc Server default ROOT web application must be removed.'
  ...
  tag gid: 'V-TCSV-00-000142'
  tag rid: 'SV-TCSV-00-000142'
  tag stig_id: 'TCSV-00-000142'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

  describe 'Release Notes txt file must not be present in core location' do
    subject { file("#{input('catalinaHome')}/webapps/ROOT/RELEASE-NOTES.txt").exist? }
    it { should eq false }
  end
  ...
  if file("#{input('catalinaHome')}/webapps/ROOT/index.jsp").exist?
    describe 'Sample content must be removed from core location - checking index.jsp file' do
      subject { file("#{input('catalinaHome')}/webapps/ROOT/index.jsp").content }

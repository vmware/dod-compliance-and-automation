control 'VRPP-8X-000041' do
  ...
  desc  'rationale', ''
  desc  'check', "
    For PostgreSQL installations that are not accessible over the network and do not have SSL turned on, this control is Not Applicable.
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"SHOW ssl_key_file;\\\"\"
  ...
  if ssl_enabled
    ...
    pg_ssl_key = command("su - postgres -c '/opt/vmware/vpostgres/current/bin/psql -A -t -c \"SHOW ssl_key_file;\"'").stdout.strip

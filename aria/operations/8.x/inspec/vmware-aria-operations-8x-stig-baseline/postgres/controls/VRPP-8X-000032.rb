control 'VRPP-8X-000032' do
  ...
  "
  desc 'fix', "
    As a database administrator, perform the following at the command prompt for each item found:
  ...
  command("su - postgres -c '/opt/vmware/vpostgres/current/bin/psql -A -t -c \"select extname from pg_extension;\"'").stdout.strip.split.each do |ext|
    describe ext do

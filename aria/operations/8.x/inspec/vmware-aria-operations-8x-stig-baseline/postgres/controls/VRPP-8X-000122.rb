control 'VRPP-8X-000122' do
  ...
  desc  'check', "
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"SHOW log_destination;\\\"\"
  ...
  desc 'fix', "
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"ALTER SYSTEM SET log_destination = 'syslog';\\\"\"
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"ALTER SYSTEM SET syslog_facility = 'LOCAL0';\\\"\"
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"ALTER SYSTEM SET syslog_ident = 'postgres';\\\"\"
  ...
  if syslog_enabled
    describe command("su - postgres -c '/opt/vmware/vpostgres/current/bin/psql -A -t -c \"SHOW log_destination;\"'") do

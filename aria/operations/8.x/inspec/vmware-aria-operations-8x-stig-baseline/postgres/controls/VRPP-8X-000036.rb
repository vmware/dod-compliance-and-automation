control 'VRPP-8X-000036' do
  ...
  desc  'check', "
    ...
    # grep -v \"^#\" /storage/db/vcops/vpostgres/repl/pg_hba.conf | grep '\\S'
  ...
  tag gid: 'V-VRPP-8X-000036'
  tag rid: 'SV-VRPP-8X-000036'
  tag stig_id: 'VRPP-8X-000036'
  tag cci: %w(CCI-000197 CCI-000764)
  tag nist: ['IA-2', 'IA-5 (1) (c)']

  describe postgres_hba_conf("#{input('pg_data_dir')}/pg_hba.conf").where { type == 'local' } do
    its('auth_method') { should_not be_in %w(trust password) }
  end

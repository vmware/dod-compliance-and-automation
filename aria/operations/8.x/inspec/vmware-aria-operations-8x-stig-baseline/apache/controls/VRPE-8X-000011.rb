control 'VRPE-8X-000011' do
  title "The vRealize Operations Manager Apache server must only allow authenticated system administrators or the designated PKI Sponsor to access the web server's private key."
  desc  "
  ...
  command('find /storage/vcops/user/conf/ssl/*key.pem -type f -xdev').stdout.split.each do |fname|
    describe file(fname) do
      ...
      its('owner') { should be_in %w(admin postgres) }
    end
  end
end

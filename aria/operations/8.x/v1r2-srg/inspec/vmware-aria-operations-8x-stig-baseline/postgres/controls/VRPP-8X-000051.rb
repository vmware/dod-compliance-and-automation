control 'VRPP-8X-000051' do
  ...
  desc 'fix', "
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"ALTER SYSTEM SET <name> TO 'on';\\\"\"
  ...
  describe command("su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -A -t -c \\\"SELECT name,setting FROM pg_settings WHERE name IN ('fsync','full_page_writes','synchronous_commit');\\\"\"") do

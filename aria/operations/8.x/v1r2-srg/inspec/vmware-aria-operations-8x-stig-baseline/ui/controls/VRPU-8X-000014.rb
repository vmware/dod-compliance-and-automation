control 'VRPU-8X-000014' do
  title 'The UI service must produce log records containing sufficient information regarding event details.'
  desc  "
  ...
  xmlconf['//*/Valve[@className="org.apache.catalina.valves.AccessLogValve"]/@pattern'].each do |pattern|
    ...
    input('ui-accessLogPattern').each do |item|
      describe pattern do
  ...
  input('ui-contextXmlFiles').each do |context|

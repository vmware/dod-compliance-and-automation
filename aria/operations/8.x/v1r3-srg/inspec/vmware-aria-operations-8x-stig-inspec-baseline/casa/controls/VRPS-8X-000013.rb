control 'VRPS-8X-000013' do
  title 'The VMware Aria Operations Casa service must initiate session logging upon startup.'
  desc  "
  ...
  desc  'check', "
    ...
    # grep -B10 -A2 '\"$CATALINA_OUT\" 2>&1 \"&\"' /usr/share/tomcat/bin/catalina.sh
  ...
  "
  desc 'fix', "
    Edit the /usr/share/tomcat/bin/catalina.sh file.
  ...
  tag gid: 'V-VRPS-8X-000013'
  tag rid: 'SV-VRPS-8X-000013'
  tag stig_id: 'VRPS-8X-000013'
  tag cci: ['CCI-001464']
  tag nist: ['AU-14 (1)']

  # look for the startup bootstrap line, and grab the 10 lines before and 2 lines after for comparison
  describe command("grep -B10 -A2 'org.apache.catalina.startup.Bootstrap \"$@\" start ' #{input('casa-tcCore')}/bin/catalina.sh") do

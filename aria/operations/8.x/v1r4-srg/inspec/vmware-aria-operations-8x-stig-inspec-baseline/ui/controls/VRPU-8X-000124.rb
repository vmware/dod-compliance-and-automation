control 'VRPU-8X-000124' do
  title 'The VMware Aria Operations UI service must enable "STRICT_SERVLET_COMPLIANCE".'
  desc  "
    Strict Servlet Compliance forces Tomcat to adhere to standards specifications including but not limited to RFC2109. RFC2109 sets the standard for HTTP session management. This setting affects several other settings which primarily pertain to cookie headers, cookie values, and sessions. Cookies will be parsed for strict adherence to specifications.
  ...
  desc  'check', "
    ...
    # grep STRICT_SERVLET_COMPLIANCE /usr/lib/vmware-vcops/tomcat-web-app/conf/catalina.properties
  ...
  desc 'fix', "
    ...
    Navigate to and open the /usr/lib/vmware-vcops/tomcat-web-app/conf/context.xml file.

    Add or update the \"xmlValidation\" attribute on the \"Context\" node:
  ...
  tag gid: 'V-VRPU-8X-000124'
  tag rid: 'SV-VRPU-8X-000124'
  tag stig_id: 'VRPU-8X-000124'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']

  describe parse_config_file("#{input('ui-catalinaPropsPath')}").params['org.apache.catalina.STRICT_SERVLET_COMPLIANCE'] do
    it { should cmp 'true' }
  end
end

control 'VRPU-8X-000001' do
  title 'The VMware Aria Operations UI service must limit the number of maximum concurrent connections permitted.'
  desc  "
  ...
  desc  'check', "
    ...
    # xmllint --xpath \"//Executor[not(@maxThreads)]/@name | //Executor[@maxThreads != '200']/@name\" /usr/lib/vmware-vcops/tomcat-web-app/conf/server.xml | awk 1 RS=' '
    # xmllint --xpath \"//Connector[not(@maxThreads)]/@port | //Connector[@maxThreads != '200']/@port\" /usr/lib/vmware-vcops/tomcat-web-app/conf/server.xml | awk 1 RS=' '
  ...
  tag gid: 'V-VRPU-8X-000001'
  tag rid: 'SV-VRPU-8X-000001'
  tag stig_id: 'VRPU-8X-000001'
  tag cci: ['CCI-000054']
  tag nist: ['AC-10']

  # Open server.xml file and get the input variable value
  xmlconf = xml(input('ui-serverXmlPath'))
  mt = input('ui-maxThreads')

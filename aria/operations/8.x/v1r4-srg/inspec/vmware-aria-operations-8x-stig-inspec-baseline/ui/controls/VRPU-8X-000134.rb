control 'VRPU-8X-000134' do
  title 'The VMware Aria Operations UI service shutdown port must be disabled.'
  ...
  desc  'check', "
    ...
    # xmllint --xpath \"//Server/@port\" /usr/lib/vmware-vcops/tomcat-web-app/conf/server.xml
  ...
  desc 'fix', "
    ...
    /usr/lib/vmware-vcops/tomcat-web-app/conf/catalina.properties

    Add or modify the setting \"base.shutdown.port=-1\" in the \"catalina.properties\" file.

    Navigate to and open:

    /usr/lib/vmware-vcops/tomcat-web-app/conf/server.xml
  ...
  tag gid: 'V-VRPU-8X-000134'
  tag rid: 'SV-VRPU-8X-000134'
  tag stig_id: 'VRPU-8X-000134'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
  describe parse_config_file("#{input('ui-catalinaPropsPath')}").params['base.shutdown.port'] do
    it { should cmp '-1' }
  end
  describe xml("#{input('ui-serverXmlPath')}") do

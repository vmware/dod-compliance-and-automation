control 'VRPU-8X-000062' do
  title 'The VMware Aria Operations UI service must be built to fail to a known safe state if system initialization fails, shutdown fails, or aborts fail.'
  ...
  desc  'check', "
    ...
    # grep -i EXIT_ON_INIT_FAILURE /usr/lib/vmware-vcops/tomcat-web-app/conf/catalina.properties
  ...
  tag gid: 'V-VRPU-8X-000062'
  tag rid: 'SV-VRPU-8X-000062'
  tag stig_id: 'VRPU-8X-000062'
  tag cci: ['CCI-001190']
  tag nist: ['SC-24']

  describe parse_config_file("#{input('ui-catalinaPropsPath')}").params['org.apache.catalina.startup.EXIT_ON_INIT_FAILURE'] do
    it { should cmp 'true' }
  end
end

control 'VRPU-8X-000152' do
  title 'The VMware Aria Operations UI service must enable "ENFORCE_ENCODING_IN_GET_WRITER".'
  ...
  desc  'check', "
    ...
    # grep ENFORCE_ENCODING_IN_GET_WRITER /usr/lib/vmware-vcops/tomcat-web-app/conf/catalina.properties
  ...
  tag gid: 'V-VRPU-8X-000152'
  tag rid: 'SV-VRPU-8X-000152'
  tag stig_id: 'VRPU-8X-000152'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']

  describe parse_config_file("#{input('ui-catalinaPropsPath')}").params['org.apache.catalina.connector.response.ENFORCE_ENCODING_IN_GET_WRITER'] do

control 'VRPS-8X-000144' do
  title 'The VMware Aria Operations Casa service files must have permissions in an out-of-the-box state.'
  ...
  desc  'check', "
    ...
    # find /usr/lib/vmware-casa/casa-webapp/conf -xdev -type f -a '(' -perm -o+w -o -not -user admin -o -not -group admin ')' -exec ls -ld {} \\;
  ...
  tag gid: 'V-VRPS-8X-000144'
  tag rid: 'SV-VRPS-8X-000144'
  tag stig_id: 'VRPS-8X-000144'
  tag cci: ['CCI-001813']
  tag nist: ['CM-5 (1) (a)']

  command("find '#{input('casa-tcInstance')}/conf' -type f -xdev").stdout.split.each do |fname|

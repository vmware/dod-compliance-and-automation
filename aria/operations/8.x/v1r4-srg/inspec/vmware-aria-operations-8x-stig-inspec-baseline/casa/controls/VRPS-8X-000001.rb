control 'VRPS-8X-000001' do
  title 'The VMware Aria Operations Casa service must limit the number of maximum concurrent connections permitted.'
  desc  "
  ...
  desc  'check', "
    ...
    # xmllint --xpath \"//Executor[not(@maxThreads)]/@name | //Executor[@maxThreads != '200']/@name\" /usr/lib/vmware-casa/casa-webapp/conf/server.xml | awk 1 RS=' '
    # xmllint --xpath \"//Connector[not(@maxThreads)]/@port | //Connector[@maxThreads != '200']/@port\" /usr/lib/vmware-casa/casa-webapp/conf/server.xml | awk 1 RS=' '
  ...
  tag gid: 'V-VRPS-8X-000001'
  tag rid: 'SV-VRPS-8X-000001'
  tag stig_id: 'VRPS-8X-000001'
  tag cci: ['CCI-000054']
  tag nist: ['AC-10']

  # Open server.xml file and get the input variable value
  xmlconf = xml(input('casa-serverXmlPath'))
  mt = input('casa-maxThreads')

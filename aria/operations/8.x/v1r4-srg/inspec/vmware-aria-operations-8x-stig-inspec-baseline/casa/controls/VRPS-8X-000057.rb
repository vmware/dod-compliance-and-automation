control 'VRPS-8X-000057' do
  title 'The VMware Aria Operations Casa service must be configured to limit data exposure between applications.'
  ...
  desc  'check', "
    ...
    # grep RECYCLE_FACADES /usr/lib/vmware-casa/casa-webapp/conf/catalina.properties
  ...
  tag gid: 'V-VRPS-8X-000057'
  tag rid: 'SV-VRPS-8X-000057'
  tag stig_id: 'VRPS-8X-000057'
  tag cci: ['CCI-000366', 'CCI-001664']
  tag nist: ['CM-6 b', 'SC-23 (3)']

  describe parse_config_file("#{input('casa-catalinaPropsPath')}").params['org.apache.catalina.connector.RECYCLE_FACADES'] do
    it { should cmp 'true' }
  end
end

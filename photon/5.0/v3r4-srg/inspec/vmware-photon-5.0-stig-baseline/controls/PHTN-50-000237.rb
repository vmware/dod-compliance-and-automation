control 'PHTN-50-000237' do
  ...
  # Compare the two files with empty lines removed and stripped of whitespace.
  aidetemplate = inspec.profile.file('aide.conf').lines.map(&:rstrip).reject(&:empty?).join("\n").strip
  aidecontent = file('/etc/aide.conf').content.lines.map(&:rstrip).reject(&:empty?).join("\n").strip

  describe 'Checking AIDE configuration' do
    subject { aidecontent }
    it { should eq aidetemplate }
  end

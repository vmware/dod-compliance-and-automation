...
*.info @<syslog server>:port;RSYSLOG_syslogProtocol23Format
...
*.<priority> @<syslog server>:port;RSYSLOG_syslogProtocol23Format

For TCP

*.<priority> @@<syslog server>:port;RSYSLOG_syslogProtocol23Format

Substitute <priority> with one of the following keywords, in ascending order of
severity and descending order of volume: debug, info, notice, warning , err,
crit, alert, emerg.

For example:

*.info @@192.168.1.1:514;RSYSLOG_syslogProtocol23Format
...
\"Edit\" in the top right. Configure a remote syslog server and click \"OK\"."
  ...
  describe file ('/etc/vmware-syslog/syslog.conf') do
    its ('content') { should match /^.*info @#{input('syslogServer')}.*$/ }
  end

...
Set-AdvancedSetting -Value true"
  ...
  # results = powercli_command("Get-VM -Name #{input('vmName')} | Get-AdvancedSetting -Name isolation.tools.copy.disable | Select Name,Value | ConvertTo-Json")
  # if !results.stdout.empty?
  #   describe json(content: results.stdout) do
  #     its(['Value']) {should cmp "true"}
  #   end
  # else
  #   describe "Advanced setting not found on VM" do
  #     subject {results.stdout}

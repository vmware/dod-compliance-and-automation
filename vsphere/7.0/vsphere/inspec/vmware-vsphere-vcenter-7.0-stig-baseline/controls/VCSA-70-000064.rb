...
where VM encryption and/or vSAN encryption is in use. Catastrophic data loss
can result from a poorly administered cryptography."
  desc  'rationale', ''
  desc  'check', "
...
Verify that only the Administrator and any site-specific cryptographic group(s)
...
the following command:
    ...
    ForEach($role in $roles){
        ...
        If($privileges -match \"Crypto*\" -or $privileges -match
\"Global.Diagnostics\" -or $privileges -match \"Host.Inventory.Add*\" -or
$privileges -match \"Host.Local operations.Manage user groups\"){
...
the following permissions from any group other than Administrator and any
site-specific cryptographic group(s):

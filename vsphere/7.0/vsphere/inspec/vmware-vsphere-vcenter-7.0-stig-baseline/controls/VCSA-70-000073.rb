...
attempt to create a persistent method of reestablishing access. One way to
accomplish this is for the attacker to simply create a new account. They may
also try to hijack an existing account by changing a password or by enabling a
previously disabled accountt. As such, all actions performed on accounts in the
SSO domain much be alerted on in vCenter at a minimum and ideally on a SIEM as
well.

    To ensure the appropriate personnel are alerted about SSO account actions,
create a new vCenter alarm for the \"com.vmware.sso.PrincipalManagement\" event
ID and configure the alert mechanisms appropriately.
  "
  desc  'rationale', ''
  desc  'check', "
...
to alert upon all SSO account actions. The alarm name may vary but it is
suggested to name it \"SSO account actions -
...
\"com.vmware.sso.PrincipalManagement\"} | Select
...
optional description. From the \"Target type\" dropdown menu, select \"vCenter
Server\". Click \"Next\".

    Paste \"com.vmware.sso.PrincipalManagement\" (without quotes) in the line
after \"IF\" and hit enter. Next to \"Trigger the alarm and\" select \"Show as
Warning\". Configure the desired notification actions that will inform the SA
and ISSO of the event. Click \"Next\". Click \"Next\". Click \"Create\".
  "
  impact 0.5
  tag severity: 'medium'

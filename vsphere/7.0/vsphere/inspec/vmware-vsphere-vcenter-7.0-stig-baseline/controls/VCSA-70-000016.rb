...
collectors."
  desc  "The distributed virtual switch can export NetFlow information about
traffic crossing the switch. NetFlow exports are not encrypted and can contain
information about the virtual network making it easier for a MitM attack to be
executed successfully. If NetFlow export is required, verify that all NetFlow
...
Configure >> Settings >> NetFlow. View the NetFlow pane and verify any
collector IP addresses are valid and in use for troubleshooting.
...
Name,@{N=\"NetFlowCollectorIPs\";E={$_.ExtensionData.config.IpfixConfig.CollectorIpAddress}}
    ...
    From the vSphere Client, go to Networking >> Select a distributed port
group >> Manage >> Settings >> Policies. Go to Monitoring and view the NetFlow
status.
...
this is a finding.
  ...
  desc  'fix', "
    ...
    From the vSphere Client, go to Networking >> Select a distributed switch >>
Configure >> Settings >> NetFlow. Click \"Edit\". Remove any unknown collector
IPs.
...
the following commands:
    ...
    Note: This will reset the NetFlow collector configuration back to the
defaults.

    To disable NetFlow on a distributed port group do the following:
    From the vSphere Client, go to Networking >> Select a distributed port
group >> Configure >> Settings >> Policies. Click \"Edit\". Click the
\"Monitoring\" tab. Change \"NetFlow\" to \"Disabled\".

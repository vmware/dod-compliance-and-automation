control 'VCSA-70-000015' do
  title "The vCenter Server must set the distributed port group Promiscuous
Mode policy to reject."
  desc  "When promiscuous mode is enabled for a virtual switch all virtual
machines connected to the port group have the potential of reading all packets
across that network, meaning only the virtual machines connected to that port
group. Promiscuous mode is disabled by default on the ESXi Server, and this is
...
the \"Security\" tab. Set \"Promiscuous Mode\" to \"Reject\". Click \"OK\".

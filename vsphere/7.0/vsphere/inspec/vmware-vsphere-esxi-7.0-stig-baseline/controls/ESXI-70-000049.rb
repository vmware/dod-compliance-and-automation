...
this network.
    ...
    The Management VMkernel port group can be on a standard or distributed
virtual switch but must be on a dedicated VLAN. The Management VLAN must not be
shared by any other function and must not be accessible to anything other than
management related functions such as vCenter.
  "
  desc  'rationale', ''
  desc  'check', "
    From the vSphere Client select the ESXi host and go to Configure >>
Networking >> VMkernel adapters. Select each VMkernel adapter that is
\"Enabled\" for management traffic and, in the bottom pane, view the \"Enabled
services\".

    If there are any services enabled on any Management VMkernel adapter, this
is a finding.

    From the vSphere Client select the ESXi host and go to Configure >>
Networking >> VMkernel adapters. Review the VLAN associated with each VMkernel
that is \"Enabled\" for management traffic. Verify with the SA that they are
...
for \"Management\". Click \"OK\".

    From the vSphere Client go to Hosts and Clusters >> Select the ESXi Host >>
Configure >> Networking >> Virtual switches. Find the port group that contains
the Management VMkernel and click the '...' button next to the name. Click
\"Edit Settings\". On the \"Properties\" tab, change the \"VLAN ID\" to one
dedicated to Management traffic. Click \"OK\".
  "
  impact 0.5
  tag severity: 'medium'

...
cryptographic modules to protect the confidentiality of remote access sessions."
  desc  "ESXi runs a reverse proxy service called rhttpproxy that front ends
internal services and APIs over one HTTPS port by redirecting virtual paths to
localhost ports. This proxy implements a FIPS 140-2 validated OpenSSL
crypographic module that is in FIPS mode by default. This configuration must be
validated and maintained in order to protect the traffic that rhttpproxy
manages."
  desc  'rationale', ''
  desc  'check', "

...
4095 unless Virtual Guest Tagging (VGT) is required."
  desc  "When a port group is set to VLAN 4095, the vSwitch passes all network
frames to the attached VMs without modifying the VLAN tags. In vSphere, this is
referred to as Virtual Guest Tagging (VGT). The VM must process the VLAN
information itself via an 802.1Q driver in the OS. VLAN 4095 must only be
implemented if the attached VMs have been specifically authorized and are
capable of managing VLAN tags themselves. If VLAN 4095 is enabled
inappropriately, it may cause denial-of-service or allow a VM to interact with
...
Configure >> Networking >> Virtual switches. For each standard switch, review
the \"VLAN ID\" on each port group and verify it is not set to \"4095\".
...
port group. Click \"Edit Settings\". On the \"Properties\" tab, change the
\"VLAN ID\" to a an appropriate VLAN ID and click \"OK\".

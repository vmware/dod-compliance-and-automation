...
use any combination of TCP, UDP and SSL transports. When using SSL, the server
certificate must be validated to ensure that the host "
  desc  'rationale', ''
  desc  'check', "
...
Configure >> System >> Advanced System Settings. Select the
\"Syslog.global.logCheckSSLCerts\" value and set it to \"true\".

    Copy the PEM formatted trusted CA certificate so that is accessible to the
host and append the contents to /etc/vmware/ssl/castore.pem by running the
follow command(s):
...
host.
  ...
  command = "(Get-VMHost -Name #{input('vmhostName')}) | Get-AdvancedSetting -Name Syslog.global.logHost | Where {$_.Value -match \"ssl\"} | Select-Object -ExpandProperty Value"

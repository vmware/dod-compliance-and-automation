...
Try {
  ...
  If ($controlsenabled.ESXI70000007) {
    ...
    ForEach ($vmhost in $vmhosts) {
      ...
      If ($asetting = $vmhost | Get-AdvancedSetting -Name $name -ErrorAction Stop) {
        If ($asetting.value -eq $value) {
          Write-ToConsoleGreen "...Setting $name is already configured correctly on $vmhost"
          $unchangedcount++
        }
        Else {
          Write-ToConsoleYellow "...Setting $name was incorrectly set to $($asetting.value) on $vmhost...setting to $value"
...
Try {
  ...
  If ($controlsenabled.ESXI70000008) {
    Write-ToConsole "...Remediating STIG ID:$STIGID with Title: $Title"
    ForEach ($vmhost in $vmhosts) {
      $name = $stigsettings.issueBanner.Keys
      $value = [string]$stigsettings.issueBanner.Values
      ## Checking to see if current setting exists
      If ($asetting = $vmhost | Get-AdvancedSetting -Name $name -ErrorAction Stop) {
        If ($asetting.value -eq $value) {
          Write-ToConsoleGreen "...Setting $name is already configured correctly to $value on $vmhost"
...
Try {
  ...
  If ($controlsenabled.ESXI70000009) {
    Write-ToConsoleBlue "...Manual remediation required for STIG ID:$STIGID with Title: $Title"
    $skipcount++
  }
  Else {
    Write-ToConsoleBlue "...Skipping disabled control STIG ID:$STIGID with Title: $Title"
...
Try {
  ...
  If ($controlsenabled.ESXI70000084) {
    ...
    ForEach ($vmhost in $vmhosts) {
      ...
      If ($results.AuditRecordStorageActive -eq "true" -and $results.AuditRecordStorageCapacity -eq $stigsettings.auditRecords.size -and $results.AuditRecordStorageDirectory -eq $stigsettings.auditRecords.dir -and $results.AuditRecordRemoteTransmissionActive -eq "true") {
...
Try {
  ...
  If ($controlsenabled.ESXI70000085) {
    ...
    ForEach ($vmhost in $vmhosts) {
      ...
      Else {
        Write-ToConsoleYellow "...Configuring Syslog x509 strict verification on $($vmhost.name)"

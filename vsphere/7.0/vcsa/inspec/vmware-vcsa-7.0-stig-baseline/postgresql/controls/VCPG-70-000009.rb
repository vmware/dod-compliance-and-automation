...
misuse and compromise of the system.
    ...
    VMware Postgres client authentication configuration is configured in
pg_hba.conf. In this file are a number of lines that specify who can connect to
the service, from where and using what authentication methods. In Postgres
there is a concept of a trusted connection where a specific network mask can
connect without any authentication, to any account. This connection is termed
'trust' in pg_hba.conf and it must not be present. Out of the box, VMware
Postgres requires standard password authentication for all connections.
  "
  desc  'rationale', ''
  desc  'check', "

...
log server."
  desc  "Envoy produces a handful of logs that must be offloaded from the
...
integrity of the hosted application.
    ...
    Envoy (rhttpproxy) rsyslog configuration is included in the
\"VMware-visl-integration\" package and unpacked to
\"/etc/vmware-syslog/vmware-services-rhttpproxy.conf\". Ensuring that the
package hashes are as expected ensures that the shipped rsyslog configuration

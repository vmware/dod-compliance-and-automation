...
.SYNOPSIS 
  ...
  Version 2 Release 1
.DESCRIPTION
  -Remediates a single host or all hosts in a specified cluster based on parameters given.
  -Individual controls can be enabled/disabled in the $controlsenabled hash table below
  -Not all controls are remediated by this script. Please review the output and items skipped for manual remediation.
  -By default this script will enable the ESXi firewall if disabled and also configure each enabled service to only be accessible by the IP ranges given. Please ensure the parameter values given are correct for your environment before running so access is not lost.
  -Some fixes will require a reboot of the host to take effect and will be displayed in vCenter as reboot required.
  -If the TLS Profile is configured using this script then host must be in maintenance mode in order for that task to run.
...
.NOTES 
  ...
  .\VMware_vSphere_8.0_STIG_ESXi_Remediation.ps1 -vcenter vcentername.test.local -hostname myhost.test.local -vccred $cred -esxAdminGroup "esxAdmins2" -allowedIPs "10.0.0.0/8" -syslogServer "tcp://log.test.local:514" -ntpServers "time.test.local","time2.test.local" -esxBuild 2999999 -reportpath C:\Reports
  ...
  .PARAMETER esxiBuild
  Specify the latest ESXi build to verify hosts are patched.
#>
...
$stigsettings = [ordered]@{
  ...
  esxiLatestBuild         = $esxiBuild #ESXI-80-000221
...
$controlsenabled = [ordered]@{
  ...
  ESXI80000247 = $true  #tls server profile
  ESXI80000248 = $true  #disable key persistence
  ESXI80000249 = $true  #dcui shell access
  ESXI80000250 = $true  #BMCNetwork
}
...
Try{
  ...
  If($controlsenabled.ESXI80000247){
    Write-ToConsole "...Remediating STIG ID:$STIGID with Title: $Title"
    ForEach($vmhost in $vmhosts){
      $esxcli = Get-EsxCli -VMHost $vmhost -V2 -ErrorAction Stop
...
Try{
  ...
  If($controlsenabled.ESXI80000248){
    Write-ToConsole "...Remediating STIG ID:$STIGID with Title: $Title"
    ForEach($vmhost in $vmhosts){
      $esxcli = Get-EsxCli -VMHost $vmhost -V2 -ErrorAction Stop
...
Try{
  ...
  If($controlsenabled.ESXI80000249){
    Write-ToConsole "...Remediating STIG ID:$STIGID with Title: $Title"
    ForEach($vmhost in $vmhosts){
      $esxcli = Get-EsxCli -VMHost $vmhost -V2 -ErrorAction Stop
...
Try{
  ...
  If($controlsenabled.ESXI80000250){
    Write-ToConsole "...Remediating STIG ID:$STIGID with Title: $Title"
    ForEach($vmhost in $vmhosts){

control 'VCLU-80-000081' do
  title 'The vCenter Lookup service must off-load log records onto a different system or media from the system being logged.'
  ...
  desc  'rationale', ''
  desc  'check', "
    By default, a vmware-services-lookupsvc.conf rsyslog configuration file includes the service logs when syslog is configured on vCenter, but it must be verified.
    ...
    input(type=\"imfile\"
          File=\"/var/log/vmware/lookupsvc/lookupsvc_stream.log.stdout\"
          Tag=\"lookupsvc-std\"
          Severity=\"info\"
          Facility=\"local0\"
          deleteStateOnFileDelete=\"on\"
          reopenOnTruncate=\"on\")
    input(type=\"imfile\"
          File=\"/var/log/vmware/lookupsvc/lookupsvc_stream.log.stderr\"
          Tag=\"lookupsvc-std\"
          Severity=\"info\"
          Facility=\"local0\"
          deleteStateOnFileDelete=\"on\"
  ...
  tag gid: 'V-259050'
  tag rid: 'SV-259050r1210406_rule'
  tag stig_id: 'VCLU-80-000081'
  tag cci: ['CCI-001851']
  tag nist: ['AU-4 (1)']

  goodcontent_v1 = inspec.profile.file('vmware-services-lookupsvc-v1.conf')
  goodcontent_v2 = inspec.profile.file('vmware-services-lookupsvc-v2.conf')

  describe.one do
    describe file('/etc/vmware-syslog/vmware-services-lookupsvc.conf').content.strip do

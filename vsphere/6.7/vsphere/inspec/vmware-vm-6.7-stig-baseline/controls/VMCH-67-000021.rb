...
console, especially if many VM console sessions are open simultaneously."
  desc  'rationale', ''
  desc  'check', "
    Remote management services, such as terminal services and SSH, must be used
to interact with virtual machines. VM console access should only be granted
when remote management services are unavailable or insufficient to perform
necessary management tasks.

    Ask the SA if a VM console is used to perform VM management tasks, other
than for troubleshooting VM issues.
...
the VM console may be temporarily granted."
  impact 0.5
  tag severity: 'medium'

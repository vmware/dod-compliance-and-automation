...
group VLAN tags and verify they are not set to the native VLAN ID of the
attached physical switch.
...
If any port group is configured with the native VLAN of the attached physical
switch, this is a finding."
  desc 'fix', "From the vSphere Client select the ESXi Host and go to Configure >>
Networking >> Virtual switches. Highlight the port group where VLAN ID set to
native VLAN ID and click Edit settings (dots). Change the VLAN ID to a

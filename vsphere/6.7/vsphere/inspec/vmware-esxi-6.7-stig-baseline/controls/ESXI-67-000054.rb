...
authentication mitigates this risk."
  ...
  tag rid: "ESXI-67-000054"
  tag stig_id: "ESXI-67-000054"
  tag cci: "CCI-000366"
  tag nist: ["CM-6 b", "Rev_4"]
  desc 'check', "From the vSphere Client select the ESXi Host and go to Configure
>> Storage >> Storage Adapters >> Select the iSCSI adapter >> Properties >>
Authentication method and view the CHAP configuration and verify CHAP is
required for target and host authentication.
...
If iSCSI is used and CHAP is not set to required for both the target and host,
this is a finding.
...
Storage >> Storage Adapters >> Select the iSCSI adapter >> Properties >>
Authentication and click the Edit button. Set Authentication method to
Use bidirectional CHAP and enter a unique secret for
each traffic flow direction.

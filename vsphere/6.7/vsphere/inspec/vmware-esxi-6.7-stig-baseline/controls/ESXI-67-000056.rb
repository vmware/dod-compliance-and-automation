...
the ESXi firewall to only allow access from authorized networks."
  ...
  tag rid: "ESXI-67-000056"
  tag stig_id: "ESXI-67-000056"
  tag cci: "CCI-000366"
  tag nist: ["CM-6 b", "Rev_4"]
  desc 'check', "From the vSphere Client select the ESXi Host and go to Configure
>> System >> Firewall. Under the Firewall section click Edit and for each
enabled service click Firewall and review the allowed IPs. Check this for
Incoming and Outgoing connnections.
...
System >> Firewall. Under the Firewall section click Edit and for each enabled
service uncheck the check box to Allow connections from any IP
address, and input the site specific network(s) required. Configure
...
#This disables the allow all rule for the target service.  We are targeting the
sshServer service in this example.

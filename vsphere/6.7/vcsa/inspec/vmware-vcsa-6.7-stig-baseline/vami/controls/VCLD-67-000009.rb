control "VCLD-67-000009" do
  title "VAMI must produce log records that contain sufficient information to
establish the outcome (success or failure) of events."
...
associated with the event."
  ...
  tag gid: nil
  tag rid: "VCLD-67-000009"
  tag stig_id: "VCLD-67-000009"
  tag cci: "CCI-000134"
  tag nist: ["AU-3", "Rev_4"]
...
the setting is uncommented the format must include '%s' at a minimum. If the
...
the line."
  ...
  describe.one do
    ...
    describe command('grep "accesslog.format" /opt/vmware/etc/lighttpd/lighttpd.conf') do
      its ('stdout.strip') { should match /^accesslog.format.*%s.*$/ }
    end
  end

control 'T1FW-3X-000021' do
  ...
  desc  'rationale', ''
  desc  'check', "
    From the NSX-T Manager web interface, go to Security >> Gateway Firewall >> Gateway Specific Rules >> Choose each Tier-1 Gateway in drop-down >> Policy_Default_Infra Section >> Action.
  ...
  unless t1s.status != 200
    ...
    if t1sjson['results'] == []
      ...
      t1sjson['results'].each do |t1|
        ...
        t1id = t1json['id']
        defaultrule = http("https://#{input('nsxManager')}/policy/api/v1/infra/domains/default/gateway-policies/Policy_Default_Infra-tier0-#{t1id}/rules/default_rule",
                          method: 'GET',
                          headers: {
                            'Accept' => 'application/json',
                            'X-XSRF-TOKEN' => "#{input('sessionToken')}",

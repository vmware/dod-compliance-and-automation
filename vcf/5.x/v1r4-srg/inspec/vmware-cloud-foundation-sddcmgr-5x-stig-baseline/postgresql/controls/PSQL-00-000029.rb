control 'PSQL-00-000029' do
  ...
  desc  'check', "
    ...
    # /usr/pgsql/15/bin/psql -h localhost -U postgres -d domainmanager -x -c \"\\dt\" | grep Owner | grep -v domainmanager

    # /usr/pgsql/15/bin/psql -h localhost -U postgres -d lcm -x -c \"\\dt\" | grep Owner | grep -v lcm

    # /usr/pgsql/15/bin/psql -h localhost -U postgres -d operationsmanager -x -c \"\\dt\" | grep Owner | grep -v opsmgr

    # /usr/pgsql/15/bin/psql -h localhost -U postgres -d platform -x -c \"\\dt\" | grep Owner | grep -v platform

    # /usr/pgsql/15/bin/psql -h localhost -U postgres -d sddc_manager_ui -x -c \"\\dt\" | grep Owner | grep -v vcfui
  ...
  desc 'fix', "
    ...
    # /usr/pgsql/15/bin/psql -h localhost -U postgres -c \"ALTER TABLE <tablename> OWNER TO <ownername>;\"

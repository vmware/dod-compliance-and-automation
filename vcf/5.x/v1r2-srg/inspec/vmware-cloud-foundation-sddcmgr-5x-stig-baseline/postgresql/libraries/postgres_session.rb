  ...
  class PostgresSession < Inspec.resource(1)
    ...
    def create_psql_cmd(query, db = [])
      ...
      if @socket_path && !inspec.os.windows?
        ...
        "/usr/pgsql/13/bin/psql -d postgresql://#{@user}:#{@pass}@/#{dbs}?host=#{@socket_path} #{option_port} -A -t -w -c #{escaped_query(query)}"
      # Host in connection string establishes tcp/ip connection
        ...
        "/usr/pgsql/13/bin/psql -d postgresql://#{@user}:#{@pass}@#{@host}:#{@port}/#{dbs} -A -t -w -c #{escaped_query(query)}"
      end
    end
  end
end

control 'CFPG-4X-000006' do
  title 'The SDDC Manager PostgreSQL service must produce error logs containing sufficient information to establish what type of events occurred.'
  desc  "
  ...
  desc  'check', "
    ...
    # psql -h localhost -U postgres -A -t -c \"SHOW log_error_verbosity\"
  ...
  desc 'fix', "
    ...
    # psql -h localhost -U postgres -c \"ALTER SYSTEM SET log_error_verbosity = 'default';\"
    # psql -h localhost -U postgres -c \"SELECT pg_reload_conf();\"
  "
  impact 0.5
  tag severity: 'medium'
  tag gtitle: 'SRG-APP-000095-DB-000039'
  tag gid: 'V-CFPG-4X-000006'
  tag rid: 'SV-CFPG-4X-000006'
  tag stig_id: 'CFPG-4X-000006'
  tag cci: ['CCI-000130']
  tag nist: ['AU-3']
  ...
  describe sql.query(sqlquery) do
    its('output') { should cmp "#{input('pg_log_error_verbosity')}" }
  end
end

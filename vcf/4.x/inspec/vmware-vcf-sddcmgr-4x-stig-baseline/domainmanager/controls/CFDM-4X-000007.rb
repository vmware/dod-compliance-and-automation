control 'CFDM-4X-000007' do
  title 'The SDDC Manager Domain Manager service must not have any symbolic links in its directory to outside directories.'
  ...
  desc  'check', "
    ...
    # find /opt/vmware/vcf/domainmanager/ -type l -ls
  ...
  tag gid: 'V-CFDM-4X-000007'
  tag rid: 'SV-CFDM-4X-000007'
  tag stig_id: 'CFDM-4X-000007'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

  results = command('find /opt/vmware/vcf/domainmanager/ -type l -ls')
  if results.stdout != ''
    results.stdout.split("\n").each do |fname|
      describe fname do
        it { should match %r{->\s/opt/vmware/vcf/domainmanager} }
      end
    end
  else

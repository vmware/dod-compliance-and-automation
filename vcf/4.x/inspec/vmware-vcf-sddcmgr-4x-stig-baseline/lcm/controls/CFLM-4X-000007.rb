control 'CFLM-4X-000007' do
  title "The SDDC Manager LCM service must not have any symbolic links in it's directory to outside directories."
  ...
  desc  'check', "
    ...
    # find /opt/vmware/vcf/lcm/ -type l -ls
  ...
  tag gid: 'V-CFLM-4X-000007'
  tag rid: 'SV-CFLM-4X-000007'
  tag stig_id: 'CFLM-4X-000007'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

  results = command('find /opt/vmware/vcf/lcm/ -type l -ls')
  if results.stdout != ''
    results.stdout.split("\n").each do |fname|
      describe fname do
        it { should match %r{->\s/opt/vmware/vcf/lcm} }
      end
    end
  else

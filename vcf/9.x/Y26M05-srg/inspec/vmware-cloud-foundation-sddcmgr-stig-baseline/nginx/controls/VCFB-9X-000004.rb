control 'VCFB-9X-000004' do
  title 'The VMware Cloud Foundation SDDC Manager NGINX server must implement HTTP Strict Transport Security (HSTS) to protect the integrity of remote sessions of external server contexts.'
  ...
  desc  'check', "
    ...
    # nginx -T 2>&1 | sed -n '/\\sserver\\s{/{:a;N;/.*location/!ba;/.*listen.*ssl/p}' | grep Strict-Transport-Security
  ...
  tag gid: 'V-VCFB-9X-000004'
  tag rid: 'SV-VCFB-9X-000004'
  tag stig_id: 'VCFB-9X-000004'
  tag cci: ['CCI-001453']
  tag nist: ['AC-17 (2)']

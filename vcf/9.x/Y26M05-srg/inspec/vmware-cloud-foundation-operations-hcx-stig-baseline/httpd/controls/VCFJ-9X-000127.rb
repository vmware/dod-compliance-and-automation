control 'VCFJ-9X-000127' do
  ...
  desc  'check', "
    ...
    # cat /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf | sed -n \"/^<VirtualHost\\s\\*:443>/,/<\\/VirtualHost>/p\" | awk '/<Location/,/<\\/Location>/{next} 1' | grep -i \"Header\" | grep -i \"Content-Security-Policy\"

    Example output:

    Header set Content-Security-Policy \"style-src 'self' 'unsafe-inline'; font-src 'self' data:; img-src 'self' data:; frame-ancestors 'none';\"
  ...
  desc 'fix', "
    ...
    /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf
  ...
  tag gid: 'V-VCFJ-9X-000127'
  tag rid: 'SV-VCFJ-9X-000127'
  tag stig_id: 'VCFJ-9X-000127'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
  ...
  header = command("cat #{conf} | sed -n \"/^<VirtualHost\\s\\*:443>/,/<\\/VirtualHost>/p\" | awk '/<Location/,/<\\/Location>/{next} 1' | grep -i \"Header\" | grep -i \"Content-Security-Policy\"").stdout.strip

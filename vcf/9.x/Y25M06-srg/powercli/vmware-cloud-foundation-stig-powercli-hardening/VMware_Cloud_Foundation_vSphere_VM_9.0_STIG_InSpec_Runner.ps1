<# 
  .SYNOPSIS 
    The VMware Cloud Foundation vSphere VM STIG InSpec runner script audits target VMs and
    facilitates creating accreditation artifacts that would require a more manual process
  ...
  .NOTES 
    File Name  : VMware_Cloud_Foundation_vSphere_VM_9.0_STIG_InSpec_Runner.ps1 
    Author     : Broadcom
    Version    : 1.0.0
    License    : Apache-2.0
  ...
  .EXAMPLE
  PS> ./VMware_Cloud_Foundation_vSphere_VM_9.0_STIG_InSpec_Runner -vccred $vccred -InspecPath /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/vm/ -InspecInputsFile /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/inputs-example.yml

  .EXAMPLE
  PS> ./VMware_Cloud_Foundation_vSphere_VM_9.0_STIG_InSpec_Runner -vccred $vccred -NoSafetyChecks -InspecPath /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/vm/ -InspecInputsFile /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/inputs-example.yml

  .EXAMPLE
  PS> ./VMware_Cloud_Foundation_vSphere_VM_9.0_STIG_InSpec_Runner -vccred $vccred -InspecPath /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/vm/ -InspecInputsFile /usr/share/stigs/vmware-cloud-foundation-stig-baseline/vsphere/inputs-example.yml -AttestationFile VMware_Cloud_Foundation_vSphere_VM_9.0_STIG_InSpec_Runner_Attestations_Example.yml
#>
...
param (
  ...
  [ValidateNotNullOrEmpty()]
  [string]$ProfileTitle = "VMware Cloud Foundation 9.0 Virtual Machine Application STIG Readiness Guide",
  [Parameter(Mandatory=$false,
...
$ReportPrefix = "VMware_Cloud_Foundation_vSphere_VM_9.x_STIG_InSpec_Report"
...
Try{
  Write-Header -Title "VMware Cloud Foundation VM STIG Auditing" -STIGVersion $STIGVersion -name $vcenter
}
Catch{
...
Catch{
  Write-Message -Level "ERROR" -Message "Failed to gather information on target VMs in vCenter: $vcenter"
  Write-Message -Level "ERROR" -Message $_.Exception
  Write-Message -Level "INFO" -Message "Disconnecting from vCenter Server: $vcenter"
  Disconnect-VIServer -Server $vcenter -Force -Confirm:$false
...
#Run InSpec profile against all VMs hosts found
Try{
  ForEach($vm in $vms){
    $name = $vm.Name
    ...
    $command = {inspec exec $InspecPath -t vmware:// --input vm_Name=$name vm_cluster="" vm_allvms=false --input-file $InspecInputsFile --show-progress --enhanced-outcomes --reporter=json:$reportFile}
    Write-Message -Level "INFO" -Message "Report path: $ReportPath and report file: $reportFile"
    Write-Message -Level "INFO" -Message "Running InSpec against VM: $name."
    Invoke-Command -ScriptBlock $command
    If(Get-Command saf){
      Write-Message -Level "INFO" -Message "Detected MITRE SAF CLI. Generating STIG Viewer Checklist for VM: $name"

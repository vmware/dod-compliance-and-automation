control 'VCFO-9X-000102' do
  title 'The VMware Cloud Foundation Operations for Networks Platform NGINX server must disable SSL session tickets.'
  ...
  desc  'check', "
    ...
    If the \"ssl_session_tickets\" directive is not set to \"off\" in the http context, this is a finding.

    If the \"ssl_session_tickets\" directive is configured in a server context and set to \"on\", this is a finding.
  "
  desc 'fix', "
  ...
  tag gid: 'V-VCFO-9X-000102'
  tag rid: 'SV-VCFO-9X-000102'
  tag stig_id: 'VCFO-9X-000102'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
  ...
  if !https.empty?
    https.each do |http|
      describe 'ssl_session_tickets configured in HTTP context' do
        subject { http['ssl_session_tickets'] }

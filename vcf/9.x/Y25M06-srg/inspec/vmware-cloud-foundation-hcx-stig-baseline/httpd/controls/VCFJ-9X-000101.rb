control 'VCFJ-9X-000101' do
  title 'The VMware Cloud Foundation Operations HCX Apache HTTP service must implement HTTP Strict Transport Security (HSTS) to protect the integrity of remote sessions.'
  ...
  desc  'check', "
    ...
    # cat /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf | sed -n \"/^<VirtualHost\\s\\*:443>/,/<\\/VirtualHost>/p\" | grep -i \"Header\"
  ...
  tag gid: 'V-VCFJ-9X-000101'
  tag rid: 'SV-VCFJ-9X-000101'
  tag stig_id: 'VCFJ-9X-000101'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']

control 'VCFB-9X-000090' do
  title 'The VMware Cloud Foundation SDDC Manager NGINX server must maintain the confidentiality of controlled information during transmission through the use of an approved TLS version.'
  desc  "
  ...
  desc  'rationale', ''
  desc  'check', "
    Verify TLS 1.2+ is enabled for each SSL enabled server context.
    ...
    If the \"ssl_protocol\" directive is not configured to only support TLS 1.2 or 1.3, this is a finding.
  "
  desc 'fix', "
    ...
    Add the ssl_protocols directive to the servers context, for example:
  ...
  tag gid: 'V-VCFB-9X-000090'
  tag rid: 'SV-VCFB-9X-000090'
  tag stig_id: 'VCFB-9X-000090'

control 'VCFB-9X-000019' do
  title 'The VMware Cloud Foundation SDDC Manager NGINX server log files must only be accessible by privileged users.'
  desc  "
  ...
  desc  'check', "
    ...
    If any NGINX log file is not owned by root or the vcf user, this is a finding.

    If any NGINX log file is not group owned by root or the vcf user, this is a finding.
  ...
  tag gid: 'V-VCFB-9X-000019'
  tag rid: 'SV-VCFB-9X-000019'
  tag stig_id: 'VCFB-9X-000019'
  ...
  logfiles = command("find #{nginx_log_dir}* -maxdepth 1 -type f").stdout
  if !logfiles.empty?

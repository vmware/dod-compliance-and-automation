control 'PHTN-50-000082' do
  ...
  desc  'rationale', ''
  desc  'check', "
    At the command line, determine the installed audit major version:

    # rpm -q --queryformat '%{VERSION}' audit | cut -d. -f1

    If audit < 4.x, run the following command to verify permissions on audit tools:
    ...
    # stat -c \"%n is owned by %U and group owned by %G and permissions are %a\" /usr/sbin/auditctl /usr/sbin/auditd /usr/sbin/aureport /usr/sbin/ausearch /usr/sbin/augenrules
    ...
    Note: /usr/sbin/autrace was removed in audit 4.x and is not applicable on those systems.
  ...
  audit_major = command('rpm -q --queryformat "%{VERSION}" audit').stdout.split('.').first.to_i

control 'PHTN-50-000107' do
  ...
  desc  'check', "
    ...
    Note: auid!=-1, auid!=4294967295, and auid!=unset are functionally equivalent in this check and the output of the above commands may be displayed in any of these formats.
  "
  desc  'fix', "
  ...
  if !results.empty?
    results.each do |path|
      describe auditd do
        ...
        its('lines') { should include /-a always,exit -S all -F path=#{path} -F perm=x -F auid>=1000 -F auid!=(?:unset|-1|4294967295) -F key=privileged/ }
      end
    end
  else

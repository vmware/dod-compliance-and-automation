...
## Requirements
- [Chef InSpec](https://downloads.chef.io/tools/inspec) or [CINC Auditor](https://cinc.sh/start/auditor/) installed on a machine with network access to the target. Tested with version 7.0.107-1. Chef/CINC Workstation can also be installed.
- SSH access to Photon instance.

## Supported Versions
- VCF 9.1.1.0 Photon 5 based appliances.

## Audit Rules
This profile includes two audit rules files to support different versions of the `audit` package:

- `audit-v3.STIG.rules` — for systems running audit 3.x
- `audit-v4.STIG.rules` — for systems running audit 4.x

To check which version of `audit` is installed on the target system, run:
```
rpm -q --queryformat '%{VERSION}' audit | cut -d. -f1
```
Or to see the full installed package version:
```
rpm -q audit
```
Use the output to determine which rules file applies before deploying audit rules to the target.

  ...
  class PostgresSession < Inspec.resource(1)
    ...
    def create_psql_cmd(query, db = [])
      ...
      if @socket_path && !inspec.os.windows?
        ...
        "/opt/vmware/vpostgres/current/bin/psql -d postgresql://#{@user}:#{@pass}@/#{dbs}?host=#{@socket_path} #{option_port} -A -t -w -c #{escaped_query(query)}"
      # Host in connection string establishes tcp/ip connection
        ...
        "su - #{@user} -c \"/opt/vmware/vpostgres/current/bin/psql -d postgresql://#{@user}:#{@pass}@#{@host}:#{@port}/#{dbs} -A -t -w -c \"#{query}\"\""
      else
        # For VCF Operations Unix user must match db user, so need to use su to run command, generally as 'postgres'
        "su - #{@user} -c \"/opt/vmware/vpostgres/current/bin/psql -d postgresql://#{@user}:#{@pass}@#{@host}:#{@port}/#{dbs} -A -t -w -c #{escaped_query(query)}\""
      end
    end
  end
end

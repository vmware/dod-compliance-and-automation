control 'VCFI-9X-000010' do
  title 'The VMware Cloud Foundation Operations PostgreSQL service must produce logs containing sufficient information to establish what type of events occurred.'
  desc  "
  ...
  desc  'check', "
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -p 5433 -A -t -c 'SHOW log_line_prefix;'\"
  ...
  desc 'fix', "
    ...
    # su - postgres -c \"/opt/vmware/vpostgres/current/bin/psql -p 5433 -A -t -c \\\"ALTER SYSTEM SET log_line_prefix = '%m %c %x %d %u %r %p %l';\\\"\"
  ...
  tag gid: 'V-VCFI-9X-000010'
  tag rid: 'SV-VCFI-9X-000010'
  tag stig_id: 'VCFI-9X-000010'

control 'VCFI-9X-000006' do
  title 'The VMware Cloud Foundation Operations PostgreSQL service configuration files must not be accessible by unauthorized users.'
  desc  "
  ...
  tag gid: 'V-VCFI-9X-000006'
  tag rid: 'SV-VCFI-9X-000006'
  tag stig_id: 'VCFI-9X-000006'
  ...
  pg_datafiles = command("find #{input('pg_data_dir')} -type f -maxdepth 1 -name '*conf*'").stdout
  if !pg_datafiles.empty?
    pg_datafiles.split.each do |fname|
    ...
    describe 'No log files found for data instance...skipping.' do
      skip 'No log files found for data instance...skipping.'
    end
  end

  pg_replfiles = command("find #{input('pg_repl_dir')} -type f -maxdepth 1 -name '*conf*'").stdout
  if !pg_replfiles.empty?
    pg_replfiles.split.each do |fname|
      describe file(fname) do
        its('mode') { should cmp '0600' }
        its('owner') { should cmp pg_owner }
        its('group') { should cmp pg_group }
    ...
    describe 'No log files found for repl instance...skipping.' do
      skip 'No log files found for repl instance...skipping.'
    end
  end
end

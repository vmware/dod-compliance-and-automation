control 'VCFH-9X-000034' do
  title 'The VMware Cloud Foundation Operations Apache HTTP service must protect system resources and privileged operations from hosted applications.'
  desc  "
  ...
  desc  'check', "
    ...
    # grep -i -E \"User |Group \" /etc/httpd/conf/httpd.conf /etc/httpd/conf/extra/httpd-ssl.conf /usr/lib/vmware-vcopssuite/utilities/conf/vcops-apache.conf
  ...
  tag gid: 'V-VCFH-9X-000034'
  tag rid: 'SV-VCFH-9X-000034'
  tag stig_id: 'VCFH-9X-000034'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

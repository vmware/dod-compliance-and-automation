control 'VCFH-9X-000007' do
  title 'The VMware Cloud Foundation Operations Apache HTTP service must generate, at a minimum, log records for system startup and shutdown, system access, and system authentication events.'
  desc  "
  ...
  desc  'check', "
    ...
    # grep -i \"LogFormat\" /etc/httpd/conf/httpd.conf /etc/httpd/conf/extra/httpd-ssl.conf /usr/lib/vmware-vcopssuite/utilities/conf/vcops-apache.conf
  ...
  tag gid: 'V-VCFH-9X-000007'
  tag rid: 'SV-VCFH-9X-000007'
  tag stig_id: 'VCFH-9X-000007'

control 'VCFJ-9X-000030' do
  title 'The VMware Cloud Foundation Operations HCX Apache HTTP service must have Multipurpose Internet Mail Extensions (MIME) that invoke OS shell programs disabled.'
  desc  "
  ...
  desc  'check', "
    ...
    # grep -i -E \"Action|AddHandler\" /etc/httpd/conf/httpd.conf /opt/vmware/config/apache-httpd/hcx-ssl.conf /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf
  ...
  tag gid: 'V-VCFJ-9X-000030'
  tag rid: 'SV-VCFJ-9X-000030'
  tag stig_id: 'VCFJ-9X-000030'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

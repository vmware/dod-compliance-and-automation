control 'VCFJ-9X-000003' do
  title 'The VMware Cloud Foundation Operations HCX Apache HTTP service must use cryptography to protect the integrity of remote sessions.'
  ...
  desc  'check', "
    ...
    # grep -i \"SSLProtocol\" /etc/httpd/conf/httpd.conf /opt/vmware/config/apache-httpd/hcx-ssl.conf /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf
  ...
  desc 'fix', "
    ...
    /opt/vmware/config/apache-httpd/hcx-ssl.conf
  ...
  tag gid: 'V-VCFJ-9X-000003'
  tag rid: 'SV-VCFJ-9X-000003'
  tag stig_id: 'VCFJ-9X-000003'

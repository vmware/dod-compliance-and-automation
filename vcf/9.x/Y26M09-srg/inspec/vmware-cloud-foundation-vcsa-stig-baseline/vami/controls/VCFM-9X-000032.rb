control 'VCFM-9X-000032' do
  ...
  access_deny = command("#{input('lighttpdBin')} -p -f #{input('lighttpdConf')} 2>/dev/null|grep 'url.access-deny'").stdout.strip
  describe access_deny do
    it { should_not be_empty }
  end

  unless access_deny.empty?
    describe access_deny do
      it { should match(/url\.access-deny\s*=\s*\(\s*"~"\s*,\s*"\.inc"\s*\)/) }
    end
  end
end

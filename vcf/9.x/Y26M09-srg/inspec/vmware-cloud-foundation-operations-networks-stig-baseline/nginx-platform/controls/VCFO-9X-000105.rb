control 'VCFO-9X-000105' do
  title 'The VMware Cloud Foundation Operations for Networks Platform NGINX server must enable Content Security Policy.'
  ...
  desc  'check', "
    ...
    # nginx -T 2>&1 | sed -n \"/^http\\s{/,/server\\s{/p\" | grep add_header
  ...
  desc 'fix', "
    ...
    Add or update the \"add_header Content-Security-Policy\" directive for each of the server contexts, for example:
  ...
  tag gid: 'V-VCFO-9X-000105'
  tag rid: 'SV-VCFO-9X-000105'
  tag stig_id: 'VCFO-9X-000105'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
  ...
  header_value2 = input('nginx_content_security_policy2')
  ...
  locations.each do |location|
    ...
    describe "Found headers defined in location: #{location.params['_']}" do
      it "should have a #{header_name} header" do
        expect(location_headers).to include(header_value).or include(header_value2)
      end
    end
  end
end

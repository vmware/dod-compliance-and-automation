  ...
  def resolve_references(data, rel_path)
    ...
    if data.key?('include')
      data.delete('include').flatten
          ...
          .select { |x| inspec.backend.run_command("test -e #{File.dirname(x)}").exit_status == 0 }
          .map { |x| find_files(x) rescue [] }.flatten
...
class NginxParser < Parslet::Parser
  ...
  rule(:quoted_identifier) do
    str('"') >> (str('"').absent? >> any).repeat.maybe.as(:identifier) >> str('"') >> space.repeat
  end

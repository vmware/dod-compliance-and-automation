control 'VCFV-9X-000203' do
  ...
  tag gid: 'V-VCFV-9X-000203'
  tag rid: 'SV-VCFV-9X-000203'
  tag stig_id: 'VCFV-9X-000203'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
  ...
  vms = []
  advSettingName = 'tools.guestlib.enableHostInfo'
  ...
  if vms.nil? || vms.empty?
    ...
    vms.each do |vm|
      ...
      describe "The setting: #{advSettingName} on VM: #{vm}" do
        ...
        its(['Value']) { should cmp('false').or be_nil.or cmp('') }
      end
    end
  end
end

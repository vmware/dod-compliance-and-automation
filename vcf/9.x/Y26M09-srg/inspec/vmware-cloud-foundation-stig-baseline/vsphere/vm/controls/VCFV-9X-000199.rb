control 'VCFV-9X-000199' do
  ...
  desc  'check', "
    ...
    If the virtual machine advanced setting \"isolation.tools.diskWiper.disable\" exists and is not set to \"true\", this is a finding.
  ...
  tag gid: 'V-VCFV-9X-000199'
  tag rid: 'SV-VCFV-9X-000199'
  tag stig_id: 'VCFV-9X-000199'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
  ...
  vms = []
  advSettingName = 'isolation.tools.diskWiper.disable'

  unless vmName.nil? || vmName.empty?
    vms = powercli_command("Get-VM -Name '#{vmName}' | Sort-Object Name | Select-Object -ExpandProperty Name").stdout.gsub("\r\n", "\n").split("\n")
  end
  unless vmcluster.nil? || vmcluster.empty?
  ...
  if vms.nil? || vms.empty?
    ...
    vms.each do |vm|
      ...
      describe "The setting: #{advSettingName} on VM: #{vm}" do
        ...
        its(['Value']) { should cmp('true').or be_nil.or cmp('') }
      end
    end
  end
end

control 'VCFA-9X-000326' do
  ...
  if vdswitches.nil? || vdswitches.empty?
    impact 0.0
    ...
    vdswitches.each do |vds|
      ...
      if result.nil? || result.empty?
        ...
        if !resultjson['CollectorIpAddress'].nil? && !resultjson['CollectorIpAddress'].empty?
          ...
          describe "The distributed switch with name: #{vds} and setting" do
            subject { resultjson['CollectorIpAddress'] }
            if resultjson['CollectorIpAddress'].nil?
              it { should be_nil }
            else
              it { should be_empty }
            end

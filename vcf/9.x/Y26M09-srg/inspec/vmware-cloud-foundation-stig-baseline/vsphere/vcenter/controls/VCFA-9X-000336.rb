control 'VCFA-9X-000336' do
  ...
  if !libraries.nil? && !libraries.empty?
    libraries.each do |library|
      ...
      if !libinfojson['PublishInfo'].nil? && !libinfojson['PublishInfo'].empty? && libinfojson['PublishInfo']['Published'] == true
      ...
      elsif !libinfojson['PublishInfo'].nil? && !libinfojson['PublishInfo'].empty? && libinfojson['PublishInfo']['Published'] == false
        ...
        describe "Subscribed content library: #{libinfojson['Name']} found. Publishing should not be enabled." do
          subject { libinfojson['PublishInfo'] }
          if libinfojson['PublishInfo'].nil?
            it { should be_nil }
          else
            it { should be_empty }
          end

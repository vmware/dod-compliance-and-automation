control 'VCFA-9X-000370' do
  title 'VMware Cloud Foundation Operations must configure Identity Broker to compare internal information system clocks with an authoritative time server.'
  desc  "
  ...
  desc  'rationale', ''
  desc  'check', "
    If an external VCF Identity Broker appliance is not deployed, this is not applicable.

    From VCF Operations, go to Fleet Management >> Lifecycle >> VCF Management >> Overview.

    Click \"Manage\" on the identity broker capability and view the currently configured NTP servers.
  ...
  desc 'fix', "
    ...
    Click \"Manage\" on the identity broker capability.
  ...
  tag gid: 'V-VCFA-9X-000370'
  tag rid: 'SV-VCFA-9X-000370'
  tag stig_id: 'VCFA-9X-000370'
  tag cci: ['CCI-004923']
  tag nist: ['SC-45 (1) (a)']
  ...
  if input('opsnet_deployed')
    ...
    impact 0.0
    describe 'VCF Identity Broker appliance is not deployed in the target environment. This control is N/A.' do
      skip 'VCF Identity Broker appliance is not deployed in the target environment. This control is N/A.'
    end
  end
end

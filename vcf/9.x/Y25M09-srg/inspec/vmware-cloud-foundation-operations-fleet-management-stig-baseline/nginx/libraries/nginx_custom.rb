...
module Inspec::Resources
  class Nginx < Inspec.resource(1)
    ...
    %w{error_log_path http_client_body_temp_path http_fastcgi_temp_path http_log_path http_proxy_temp_path http_scgi_temp_path http_uwsgi_temp_path lock_path modules_path prefix sbin_path service version user group}.each do |property|
    ...
    # Capture only statically compiled modules
    def modules
      @data.scan(/--with-(\S+)_module(?!=dynamic)/).flatten
      #@data.scan(/--with-(\S+)_module/).flatten.concat(@data.scan(/--add-module=(\S+)/).flatten)
    end

control 'VCFQ-9X-000100' do
  title 'The VMware Cloud Foundation Operations Fleet Management NGINX server must not disable keep alive timeouts.'
  desc  "
  ...
  tag gid: 'V-VCFQ-9X-000100'
  tag rid: 'SV-VCFQ-9X-000100'
  tag stig_id: 'VCFQ-9X-000100'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
    ...
    https.each do |http|
      if http['keepalive_timeout']
        describe 'keepalive_timeout configured in HTTP context' do
          ...
          subject { http['keepalive_timeout'].join.tr('s', '') }
          it { should cmp > 0 }
          it { should cmp <= 75 }
        end
      else
    ...
    servers.each do |server|
      if server.params['keepalive_timeout']
        describe "Found keepalive_timeout defined in server context with listener: #{server.params['listen']}" do
          # Trim off the s off the end of the value
          subject { server.params['keepalive_timeout'].join.tr('s', '') }
          it { should cmp > 0 }
          it { should cmp <= 75 }
        end
      else

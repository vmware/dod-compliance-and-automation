control 'VCFT-9X-000134' do
  title 'The VMware Cloud Foundation Operations for Logs Loginsight service must have the http-only flag set for cookies.'
  ...
  desc  'check', "
    ...
    If the \"http-only\" parameter under \"cookie-config\" is not set to \"true\", this is a finding.
  "
  desc 'fix', "
    ...
    Navigate to the <session-config> node and configure the <http-only> setting as follows:
  ...
  tag gid: 'V-VCFT-9X-000134'
  tag rid: 'SV-VCFT-9X-000134'
  tag stig_id: 'VCFT-9X-000134'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']

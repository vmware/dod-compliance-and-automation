control 'VCFO-9X-000025' do
  title 'The VMware Cloud Foundation Operations for Networks Platform NGINX server must not perform user management for hosted applications.'
  desc  "
  ...
  desc  'check', "
    ...
    #  nginx -T 2>&1 | grep -E \"^\\s+auth_basic\"
  ...
  tag gid: 'V-VCFO-9X-000025'
  tag rid: 'SV-VCFO-9X-000025'
  tag stig_id: 'VCFO-9X-000025'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']

  describe command('nginx -T 2>&1 | grep -E "^\s+auth_basic"') do

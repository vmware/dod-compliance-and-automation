control 'VCFJ-9X-000009' do
  title 'The VMware Cloud Foundation Operations HCX Apache HTTP service must produce log records containing sufficient information to establish what type of events occurred.'
  desc  "
  ...
  desc  'check', "
    ...
    # grep -i -E \"ErrorLog|LogLevel\" /etc/httpd/conf/httpd.conf /opt/vmware/config/apache-httpd/hcx-ssl.conf /opt/vmware/config/apache-httpd/hcx-virtual-hosts.conf

    Example output:

    ErrorLog \"/common/logs/httpd/error.log\"
    LogLevel info

    If the value of \"ErrorLog\" is not configured to \"/common/logs/httpd/error.log\", this is a finding.
  ...
  tag gid: 'V-VCFJ-9X-000009'
  tag rid: 'SV-VCFJ-9X-000009'
  tag stig_id: 'VCFJ-9X-000009'
  tag cci: ['CCI-000130']
  tag nist: ['AU-3 a']
  ...
  if apache_conf_custom(conf).ErrorLog.nil?
    ...
    describe apache_conf_custom(conf) do
      its('ErrorLog') { should cmp apache_error_log }
    end
  end

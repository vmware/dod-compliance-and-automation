control 'PSQL-00-000041' do
  ...
  if ssl_enabled
    ...
    sql_result = command("su - postgres -c '/opt/vmware/vpostgres/current/bin/psql -A -t -c \"SHOW ssl_key_file;\"'")

    describe "Key File - '#{sql_result.stdout.strip}'" do
      subject { sql_result.stdout.strip }
      it { should cmp pg_ssl_key }
    end
